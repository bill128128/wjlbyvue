	...
	export default{
		...
		data(){
			return{
				...
				pnum : 9,
				dataVal:new Date(),
				agentType:4, //状态，1：省，2：市，3：县，4：平级
		...
		methods:{
            ...
            closeTouch:function(){        
                document.getElementById("levelingAgentDetail").addEventListener('touchmove',this.handler,{passive:false}); //阻止默认事件   
            },    
            openTouch:function(){        
                document.getElementById("levelingAgentDetail").removeEventListener('touchmove', this.handler,{passive:false}); //打开默认事件     
			},
			 openPicker(){

	...
	export default{
		...
		data(){
			return{
				...
				pnum : 9,
				dataVal:new Date(),
				agentType:2, //状态，1：省，2：市，3：县，4：平级
		...
		methods:{
            ...
            closeTouch:function(){        
                document.getElementById("municipalAgentDetail").addEventListener('touchmove',this.handler,{passive:false}); //阻止默认事件   
            },    
            openTouch:function(){        
                document.getElementById("municipalAgentDetail").removeEventListener('touchmove', this.handler,{passive:false}); //打开默认事件     
            },
            openPicker(){

...
<script>
	...
	export default{
		...
		methods:{
			...
			getlist(){
				this.$store.dispatch('ZJBDailyReturn/dailyreturn',this.page).then(()=>{
					if(this.$store.state.ZJBDailyReturn.code == 200){
						this.allLoaded = false;
						if(this.pageval == this.$store.state.ZJBDailyReturn.list.length){
							this.list = this.$store.state.ZJBDailyReturn.list;
						}else{
							this.list = this.$store.state.ZJBDailyReturn.list;
							this.allLoaded = true;
						}						
						//this.list = this.list.concat(this.$store.state.ZJBDailyReturn.list);
						this.member_score = this.$store.state.ZJBDailyReturn.member_score;
						this.bill_sum = this.$store.state.ZJBDailyReturn.bill_sum;
					}else{
						this.$toast({
							message:this.$store.state.ZJBDailyReturn.msg,
						});
					}
				}).catch((e) => {              
					  //alert(e);

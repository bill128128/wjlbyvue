// 直推奖明细
<template>
	<div id="moveJZBListin">
        <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore" :bottomDistance='bottomDistance'>
            <!-- 4:list列表明细 -->
            <ul class="list_menu">
                <li class="item" v-for="(item,index) in list" v-bind:key="index">
                    <div class="user_info">
                        <span><span  class="user">{{item.nickname}}</span>&nbsp;&nbsp;(转入用户)</span>
...
<script>
	...
	export default{
		...
		methods:{
            ...
            getin(){
            	this.$store.dispatch('conversionAssetsList/getConversionAssetsinDetail',this.info).then(()=>{

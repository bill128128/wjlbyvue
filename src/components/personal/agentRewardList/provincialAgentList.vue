<template>
	<div id="provincialAgentList">
		<main class="main_available">
            <div class="item">
                <div class="count">{{this.$store.state.agentRewardList.pro.number}}</div>
                <h3>代理人数</h3>
            </div>
            <div class="item">
                <div class="count">{{this.$store.state.agentRewardList.pro.Int}}</div>
                <h3>总计代理奖励</h3>
            </div>
	...
	export default{
		...
		data(){
			return{
                ...
                dataVal:new Date(),
                page:1,
                agentListType:1,//状态，1：省，2：市，3：县，4：平级,
//              num:0,
//              Int:'',
			}
		},

<template>
	<div id="countyLeveAgentList">
		<main class="main_available">
            <div class="item">
                <div class="count">{{this.$store.state.agentRewardList.county.number}}</div>
                <h3>代理人数</h3>
            </div>
            <div class="item">
                <div class="count">{{this.$store.state.agentRewardList.county.Int}}</div>
                <h3>总计代理奖励</h3>
            </div>
	...
	export default{
		...
		data(){
			return{
                ...
                dataVal:new Date(),
                page:1,
                agentListType:3//状态，1：省，2：市，3：县，4：平级
            }
        },
        mounted(){
...
<style scoped>
    ...
    /* 时间选择器 */
    .picker-item{ font-size:0.37rem;}
    .mint-datetime-action{font-size:0.37rem;}
</style>
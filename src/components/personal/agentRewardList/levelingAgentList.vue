<template>
	<div id="levelingAgentList">
		<main class="main_available">
            <div class="item">
                <div class="count">{{this.$store.state.agentRewardList.level.number}}</div>
                <h3>代理人数</h3>
            </div>
            <div class="item">
                <div class="count">{{this.$store.state.agentRewardList.level.Int}}</div>
                <h3>总计代理奖励</h3>
            </div>
	...
	export default{
		...
		data(){
			return{
                ...
                dataVal:new Date(),
                page:1,
                agentListType:4//状态，1：省，2：市，3：县，4：平级
			}
		},
        mounted(){
            ...
            getagentRewardList(){
                this.$store.dispatch('agentRewardList/getagentRewardList',this.page).then(() =>{
                    if(this.$store.state.agentRewardList.code == 200){
                        this.agentList = this.agentList.concat(this.$store.state.agentRewardList.level);
                    }else{

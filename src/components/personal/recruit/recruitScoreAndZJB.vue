<template>
    <div id='recruitScoreAndZJB'>
        <div class="main_available">
            <div class="item">
                <div class="count">{{recruitScore.haveRecruitScore}}</div>
                <h3>可用招募积分</h3>
            </div>
            <div class="item">
                <div class="count">{{recruitScore.haveZjb}}</div>
                <h3>可用掌聚宝</h3>
            </div>
        </div>
        <form action="">
                ...
                <div class="selected" v-show="!isShowleaseIntegral">
                    <div class="placeholder">招募积分：
                        <input type="text" class="count" value="" v-model="info.frozenScore">
                    </div>
                </div>
                <div class="selected" v-show="!isShowleaseIntegral">
                    <div class="placeholder">掌聚宝：{{info.leaseIntegralCount - info.frozenScore}}
                    </div>
                </div>
...
<script>
    export default{
		...
		methods:{
            ...
            getRecruitScore(){  //获取信息
                this.$store.dispatch('recruit/getRecruitScore').then(()=>{
				if(this.$store.state.recruit.recruitcode==200){
                    this.recruitScore.haveRecruitScore = this.$store.state.recruit.frozen_score ;
                    this.recruitScore.haveZjb = this.$store.state.recruit.release_score ;
				}else{
					this.$toast({
            ...
            changeScore:function(){
                if(this.info.leaseIntegralCount == "" || this.info.leaseIntegralCount == null){
                    ...
                    this.$toast({message:'填写的招募积分数量不能大于租赁积分'})
                    return false;
                }else if(this.info.leaseIntegralCount - this.info.frozenScore>this.recruitScore.haveZjb){
                    this.$toast({message:'可用掌聚宝不足'})
                    return false ;
                }

// 关于我们 - 管理团队内容部分
<template>
    <div id="teamContent" class="animated zoomIn">
        <h3>{{content.title}}</h3>
        <div class="content" v-html="content.contents">{{content.contents}}</div>
    </div>
</template>
<script>
    export default{
            ...
            getContent(){
                this.$store.dispatch('knowAboutUs/getContent').then(() => {
					if(this.$store.state.knowAboutUs.code==200){
                        this.content = this.$store.state.knowAboutUs.list[2] ;
					}else{
						this.$toast({

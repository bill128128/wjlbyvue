<template>
    ...
    <div id="modifyAddress">
        <header class="top">
			<img src="../../../static/img/back.png" @click="gaBack">
            修改收货地址
            <span @click="modifyAddress">保存</span>
        </header>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            getModifyAddress(){             //获取要更改的地址
                // let message = localStorage.getItem('address'); //字符串格式，不是对象
                // console.log(typeof(message)); //string
                // console.log(message);
                let message = JSON.parse(localStorage.getItem('address'));
                this.info.userName = message.consignee ;
                this.info.userMobile = message.mobile ;
                this.region = message.address_region ;
                this.detailsAddress = message.address ;
                let reginSplit = this.region.split(" ");
                this.province = reginSplit[0];
                this.city = reginSplit[1];
                this.county = reginSplit[2];
                this.info.userId = message.address_id;
            },
            modifyAddress(){              //修改地址
                ...
                this.$store.dispatch('address/modifyAddress',[this.info,this.provinceId,this.cityId,this.countyId,this.detailsAddress]).then(() => {
                    if(this.$store.state.address.modifyAddressCode == 200){
                        this.$toast({message:this.$store.state.address.modifyAddressMsg})
                        setTimeout(() => {

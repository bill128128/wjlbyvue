            ...
            <ul class="coversionRecordList">
                <li class="coversionRecordItem" v-for="(item,index) in list" :key="index" @click="gotoProductDetails(item.goods_id)">
                    ...
                    <div class="item3">
                        ...
                        <button v-show="item.delivery == '待收货'" @click.stop="sureDeliveryGoods(item.id)">确认收货</button>
                    </div>
                </li>
            </ul>
        </mt-loadmore>
    ...
    export default{
        data(){
            return{
                ...
                info:{
                   page:1, //分页 
                   status:2 //所属类型：0为全部，1待发货，2待收货，3已完成
                },
                stype:'',//1：零售 2：批发
                allLoaded:false,
				loading:false,
        ...
        methods:{
            ...
            sureDeliveryGoods(goodsId){  //确认收货
                ...
                .then(() => {
                    if(this.$store.state.conversionRecord.makeSureDeliveryGoodsCode == 1){
